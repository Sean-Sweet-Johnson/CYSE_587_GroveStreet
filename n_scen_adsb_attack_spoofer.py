...
from spoofer import Spoofer  # Import the updated Spoofer
...
def update(frame):
    ...
    for drone in drones:
        ...
        if status == -2:
            ...
            # Step 2: Apply spoofing logic (by Spoofer class)
            received_message, spoofed = spoofer.spoof_message(received_message)
            if spoofed:
                print(f"Drone {drone.id} message spoofed with fake ID: {received_message['drone_id']}")
            ...
            # Step 3: Update GCS with the received message
            gcs.receive_update(
            ...
            # Step 4: Update drone marker position
