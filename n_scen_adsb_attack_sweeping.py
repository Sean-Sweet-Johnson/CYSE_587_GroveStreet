...
from jammer_sweeping import SweepingJammer
...
channel = ADSBChannel()
jammer = SweepingJammer(jamming_probability=0.4, noise_intensity=0.8)  # Adjust probability as needed
...
# Improve the 3D plot visualization
ax.set_xlabel("Latitude")
ax.set_ylabel("Longitude")
ax.set_zlabel("Altitude (m)")
ax.set_box_aspect([1, 1, 0.5])  # Adjust aspect ratio
...
def update(frame):
    ...
    for drone in drones:
        ...
        if status == -2:
            ...
            received_message, delay_ns, corrupted, snr_db = channel.transmit(
                copy.deepcopy(original_message), gcs_pos, jammer=jammer, spoofer=spoofer
            )
    ...
    if not active_drones:
        ...
        ani.event_source.stop()  # Stop animation instead of closing
